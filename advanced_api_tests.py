...
def test_report_after_addition():
    ...
    response = requests.get(f"{BASE_URL}/report/?id=123123&year=2025&month=5")
    print(response.status_code, response.json())
    assert response.status_code == 200, "Report request should succeed"
    report_data = response.json()
    found = False
    for category in report_data["costs"]:
        for key, value in category.items():
            if any(item["description"] == "Bread" for item in value):
                found = True
...
def run_tests():
    with open("advanced_api_test_results.txt", "w") as f:
        sys.stdout = f
        test_about()
